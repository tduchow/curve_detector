...
def callback(data):
	...
	for begin_th, end_th in good_intervals:
		...
		if 70 < mean and mean < 135 and std_dev < 15:
			...
			arc = Point32()
			arc.x = center_range*math.cos(math.degrees(center_angle))
			arc.y = center_range*math.sin(math.degrees(center_angle))
			#pc.points.append(arc)
			f = Point32()
			f.x = polar_to_rect_x(first_pt)
			f.y = polar_to_rect_y(first_pt)
			pc.points.append(f)
			l = Point32()
			l.x = polar_to_rect_x(last_pt)
			l.y = polar_to_rect_y(last_pt)
			pc.points.append(l)
...
def polar_to_rect_x(p):
	return p[1]*math.cos(math.degrees(p[0]))

def polar_to_rect_y(p):
	return p[1]*math.sin(math.degrees(p[0]))
